...
for filename, label in files_and_labels:
    with uproot.open(f"{filename}:preselection") as tree:
        ...
        cut_mask = (invM>-100) & (invM<.18) #(invM > .001*mass-.005) & (invM < .001*mass+.005)
...
plt.grid(True)
plt.tight_layout()
plt.savefig("Adversary_ROC_MulticlassTestNoMassDiscr.png")
...
def plot_roc(y_true, y_score, title):
    ...
    try:
        ...
        plt.grid(True)
        plt.tight_layout()
        helper=str(title)+"NoMassDiscr.png"
        plt.savefig(helper)
    except ValueError:
...
plt.grid(True)
plt.tight_layout()
plt.savefig("Adversary_ROC_MulticlassTrainNoMassDiscr.png")
...
plt.grid(True)
plt.savefig("SigmoidBingBongNoMassDiscr.png")
